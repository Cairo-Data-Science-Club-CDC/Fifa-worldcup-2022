...
```{r}
wc_data%>% 
  filter(cash_type == "cost") %>%view
  ggplot(aes(year,cash_value, color = event_type, group = event_type))+
  geom_point()+
  geom_line()+
  scale_y_continuous(trans = "log10")+
  facet_wrap(~cash_source)
```


```{r}
wc_data%>% 
  filter(cash_type == "revenue") %>%
  ggplot(aes(year,cash_value, color = event_type, group = event_type))+
  geom_point()+
  geom_line()+
  scale_y_continuous(trans = "log10")+
...
```{r}
wc_data %>% 
  filter(cash_type == "cost") %>%
  group_by(host_country, event_type, year) %>% 
  summarize(cost_value = mean(cash_value,  na.rm = TRUE)) %>% 
  ungroup() %>% 
  group_by(host_country) %>% 
  mutate(country_avg_cost = mean(cost_value, na.rm = TRUE)) %>% 
  ungroup() %>% 
  ...
  geom_point(aes(shape = event_type, color = year), size = 3)+
...
```{r}
wc_data %>% 
  #filter(host_country == "Germany") %>%
  group_by(host_country, event_type, year, cash_type) %>% 
  summarize(cost_value = mean(cash_value,  na.rm = TRUE)) %>% 
  ungroup() %>% 
  group_by(host_country, cash_type) %>% 
  mutate(country_avg_cost = mean(cost_value, na.rm = TRUE)) %>% 
  ungroup() %>% 
  arrange(country_avg_cost) %>% 
  mutate(host_country = fct_inorder(host_country)) %>% 
  ggplot(aes( host_country, cost_value, group = paste(host_country, year)))+
  geom_line(aes(color = event_type), position = position_dodge(width = 0.7))+
  geom_point(aes(shape = cash_type), size = 3,position = position_dodge(width = 0.7))+
  scale_y_continuous(trans = "log10")+
  coord_flip()
```
...
wc_data %>% 
  ...
  mutate(country_avg_cost = mean(cost_value, na.rm = TRUE)) %>% 
  ungroup() %>% 
  arrange(country_avg_cost) %>% view
  mutate(host_country = fct_inorder(host_country),
         cost_value = cost_value/1000,
         cost_value = ifelse(cash_type == "cost", cost_value*-1, cost_value)) %>% 
  ggplot(aes( host_country, cost_value, group = paste(host_country, year)))+
  geom_line(aes(color = event_type), position = position_dodge(width = 0.7))+
  geom_point(aes(shape = cash_type), size = 3,position = position_dodge(width = 0.7))+
  coord_flip()+
